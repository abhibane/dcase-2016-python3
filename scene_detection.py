...
def main(argv):
    ...
    print('Parameters File:', parameter_file)
...
#    elif not args.development and args.challenge:
    ...
    print('params.init:', params['flow']['initialize'])
    if params['flow']['initialize']:
        ...
        if not os.path.isfile(current_model_file) or overwrite:
            ...
            for label in data:
                progress(title_text='Train models',
                         ...
                         note=label)
                if classifier_method == 'gmm':
                    model_container['models'][label] = mixture.GaussianMixture(**classifier_params).fit(data[label])
                else:
                    raise ValueError("Unknown classifier method ["+classifier_method+"]")
...
def do_system_evaluation(dataset, result_path, dataset_evaluation_mode='folds'):
    ...
    for fold in dataset.folds(mode=dataset_evaluation_mode):
        ...
        for result in results:
            if(len(result) > 0):
                # print("Result:", result)
                audio_file = result[0].replace('\\', '/')
                #print('File meta:', dataset.file_meta(audio_file))
                
                y_true.append(dataset.file_meta(audio_file)[0]['scene_label'])
