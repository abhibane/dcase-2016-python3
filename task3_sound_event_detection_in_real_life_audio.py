            ...
            if not os.path.isfile(current_model_file) or overwrite:
                ...
                for event_label in data_positive:
                    ...
                    if classifier_method == 'gmm':
                        ...
                        model_container['models'][event_label]['positive'] = mixture.GaussianMixture(**classifier_params).fit(data_positive[event_label])
                        model_container['models'][event_label]['negative'] = mixture.GaussianMixture(**classifier_params).fit(data_negative[event_label])
                    else:
                        raise ValueError("Unknown classifier method ["+classifier_method+"]")
