...
def load_audio(filename, mono=True, fs=44100):
    ...
    if file_extension == '.wav':
        # Load audio
        audio_data, sample_rate = soundfile.read(filename, always_2d=True)
        if(len(audio_data.shape) == 1):
            audio_data = np.asarray(audio_data)
            
        audio_data = audio_data.T

        if(("QR_" in filename) or ("99.wav" in filename)):
            print(filename, '- samplerate:', sample_rate, ', audiodata:', audio_data.shape, len(audio_data.shape))
